...
/* Move robot by vx、vy */
bool SwarmRobot::moveRobotByXY(int index, double theta, double vx, double vy) {
    double const_x = 0.1;
    double const_y = 0.0;
    ...
    geometry_msgs::Twist vel_msg;
    vel_msg.linear.x = mat_trans_inv(0,0) * vx +mat_trans_inv(0,1) * vy;
    vel_msg.angular.z = mat_trans_inv(1,0) * vx +mat_trans_inv(1,1) * vy;
    cmd_vel_pub[swarm_robot_id[index]-1].publish(vel_msg);
...
/* Collision Avoidence */
bool SwarmRobot::avoidCollision(double alpha_1, double alpha_2, double rad, double df, Eigen::VectorXd cur_x, Eigen::VectorXd cur_y, Eigen::VectorXd &ac_x, Eigen::VectorXd &ac_y){
    bool flag = false;
